//- name: Симплекс метод для канонической формы
//- description: метопы
//- author: &#60;T&#62; + &#60;K&#62;
//- semester: 4
//- input: html
//- input_default_value: $$c($$<input type="text" style="width:100px" id="T1c" value="-0.5 0 0 -1 0"><br>
//- input_default_value: $$b($$<input type="text" style="width:100px" id="T1b" value="-1 -1 3"><br>
//- input_default_value: $$a\Bigg($$<textarea id="T1a" style="width:110px;height:55px;vertical-align:middle;">12 6 0 -18 0\n12 0 6 -18 0\n3 0 0 -6 3</textarea>$$\Bigg)$$<br>
...
//- input_default_value: Алгоритм: <select id="T1alg"><option>Симплекс метод для канонической формы</option><option>Двойственный симплекс метод для канонической формы</option></select><br>
//- output: html
//- import: math
//- import: nerdamer
//try{(()=>{
...
const is_dual = input.by_id('T1alg').value!='Симплекс метод для канонической формы';
...
function dual(){
	output.print('Двойственная задача имеет вид:<br>');
	const del_ind = C.map((x,i)=>x?-1:i);
	const dual_B=C.filter(x=>x);
	const dual_C=B.concat(new Array(dual_B.length).fill(0));
	const dual_A=math.transpose(A.map(x=>x.filter((x,i)=>del_ind.indexOf(i)==-1))).map(x=>x.concat(new Array(dual_B.length).fill(0))).map((x,i)=>{x[A.length+i]=find_min?1:-1;return x;});
	output.print((find_min?'max':'min')+'{$$'+_toTex(dual_C,(i)=>'y_'+i)+'$$} при ограничениях:<br>');
	
	output.print('$$\\left\\{\\begin{align}'+
	dual_A.map((x,i)=>_toTex(x,(i)=>'y_'+i)+'&='+dual_B[i]).join(' \\\\')+
		' \\\\ '+
		'y_i\\geq 0,i = 1,'+dual_A[0].length+
	'\\end{align}\\right.$$<br/>');
	
	let lables = [[],dual_A[0].map((x,i)=>'y_'+(i+1))];
	dual_B.map(x=>lables[0].unshift(lables[1].pop()));
	let matrix = {};
	dual_A.map((x,i)=>{
		matrix[lables[0][i]]={};
		x.map((y,j)=>{
	...
	//
	// к примеру column_var мб "x_3"
	function dual_select_next_row(column_var) {
		let keys = Object.keys(matrix);
		let res_val = Number.MAX_VALUE;
		let res_key = "";
		for (let row_index = 0; row_index < keys.length; row_index++) {
	...
	// 2 x + 4 y + 3 -> obj
	function dual_extract_coffs(str) {
		let obj = {};
		Array.from(str.matchAll(/[xy]_\d+/g)).map(x=>obj[x] = math.derivative(str.replace(/_/g,''), x[0].replace(/_/g,'')).evaluate());
		str=str.replace(/[xy]_\d+/g,'0');
		obj.b = math.evaluate(str);
	...
	// возвращает без "="
	function dual_stringify_row(sel_row) {
		return Object.keys(matrix[sel_row])
			.map(key => key === "b" ? matrix[sel_row].b
				: "" + key + "*" + matrix[sel_row][key])
			.join("+");
	...
	let max_iter=20;
	while (!(dual_check_is_answer_ready_max(f) || !(max_iter--))) {
		//output.print(f);
		// выбираем следующик колонку и строку
		let sel_column = dual_select_next_column_max();
		//output.print(matrix);
		let sel_row = dual_select_next_row(sel_column);
		...
		// выражаем переменную, которая была в sel_column из строки sel_row
		let eq_str = dual_stringify_row(sel_row);
		eq_str += "=" + sel_row;
		...
		for(let key of Object.keys(matrix)){
			//let row = matrix[key]
			let row_str = dual_stringify_row(key);
			row_str = row_str.replaceAll(sel_column, `(${solvedStr})`);
			let simpl = math.rationalize(math.simplify(math.rationalize(row_str))).toString();
			simpl = simpl.replace(/\d(\.\d+)?e-\d+/g,'0');
			matrix[key] = dual_extract_coffs(simpl);
		}
		f=matrix.f;
		delete matrix.f;
