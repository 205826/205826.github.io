...
output.print('<p>Не прошедшее IT-регистрацию ПАО "Ой, всё упало" в этой контрольной работе для студентов ведет разработку ПО с использованием ОСНО в 2025 году. ПАО воспользовалось услугами субподрядчиков на ОСНО, и получила документарные подтверждения указанных в задании работ, включенных в разработанное ПО. Цена ПО ПАО для заказчиков составляет ' + task1.pricePO.toLocaleString('ru-RU') + ' рублей включая НДС. Какую, рассчитанную вручную сумму НДС, должно уплатить ПАО в бюджет после продажи своего ПО?</p>');
output.print('<ul>');
output.print('<li>ООО "АйТи-Прорыв": услуг разработки ПО на ' + task1.sum1.toLocaleString('ru-RU') + ' рублей с НДС;</li>');
output.print('<li>ООО "Оптимизировали хуже": услуг разработки ПО на ' + task1.sum2.toLocaleString('ru-RU') + ' рублей с НДС;</li>');
output.print('<li>ООО "Клик-клак-код": услуг разработки ПО на ' + task1.sum3.toLocaleString('ru-RU') + ' рублей без НДС;</li>');
output.print('<li>ООО "Кэш и Соль": услуг разработки ПО на сумму, где НДС составляет ' + task1.sum4_nds.toLocaleString('ru-RU') + ' рублей;</li>');
output.print('</ul>');
...
output.print('<p>В ООО "Оптимизировали хуже" ведется разработка информационной системы инновационных самоходных хлебобулочных изделий "Колобок". Сколько стоит (без НДС) доработка системы со следующими требованиями (в чел.часах), если используется методика PERT, а стоимость одного человека-часа составляет ' + task2.costPerHour.toLocaleString('ru-RU') + ' рублей? Ответ округлить до целого.</p>');
output.print('<ul>');
...
output.print('<p>Не прошедшее IT-регистрацию ЗАО "Зашёл и завис" использует ОСНО в 2024 году. По условиям этой контрольной работы директор решил выплатить своему сотруднику премию за разработку ПО в размере ' + task5.premiaNaRuki.toLocaleString('ru-RU') + ' рублей на руки. Посчитайте вручную, как оптимизировать налоги, и сколько директор должен включить в счет для заказчика, если в этом году он уже выплатил ему ' + task5.alreadyPaid.toLocaleString('ru-RU') + ' рублей? Компания не является малым предприятием.</p>');
...
if (input.value_by_id("show_answers")) {
    ...
    output.print('<p><strong>Задание 1:</strong> ' + task1.correctAnswer.toLocaleString('ru-RU') + ' руб.</p>');
    output.print('<p><strong>Задание 2:</strong> ' + task2.correctAnswer.toLocaleString('ru-RU') + ' руб.</p>');
    output.print('<p><strong>Задание 3:</strong> ' + task3.correctAnswer.toLocaleString('ru-RU') + ' руб.</p>');
    output.print('<p><strong>Задание 4:</strong> ' + task4.correctAnswer.toLocaleString('ru-RU') + '</p>');
    output.print('<p><strong>Задание 5:</strong> ' + task5.correctAnswer.toLocaleString('ru-RU') + ' руб.</p>');
}
