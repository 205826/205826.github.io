...
output.print('С = (0,'+calcansv.toString(16).toUpperCase().padStart(6, "0").replace(/0+$/,'')+')<sub>16</sub> · 2<sup>'+shift+'</sup> = '+((calcansv/0x1000000*(16**shift)).toFixed(8).replace(/\.?0+$/, '')).replace('.',',')+'.<br>');
output.print('<br>');
output.print('Определим абсолютную и относительную погрешности результата:<br>');
output.print('ΔС =  '+ansv.toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+' – '+(calcansv/0x1000000*(16**shift)).toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+' = '+(ansv-(calcansv/0x1000000*(16**shift))).toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+'<br>');

output.print('<table style="border: none;">');
output.print('<tr>');
output.print('<td style="border: none;" rowspan="2">δС =&nbsp;</td>');
output.print('<td style="border: none;border-left:1px solid;" rowspan="2">&nbsp;</td>');
output.print('<td style="border: none;border-bottom:1px solid;">'+(ansv-(calcansv/0x1000000*(16**shift))).toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+'</td>');
output.print('<td style="border: none;border-right:1px solid;" rowspan="2">&nbsp;</td>');
var pogr = Math.abs((ansv-(calcansv/0x1000000*(16**shift)))/(ansv))*100;
...
output.print('С = (0,'+calcansv.toString(2).toUpperCase().padStart(6*4, "0").replace(/0+$/,'')+')<sub>16</sub> · 2<sup>'+shift+'</sup> = '+((calcansv/0x1000000*(2**shift)).toFixed(8).replace(/\.?0+$/, '')).replace('.',',')+'.<br>');
output.print('<br>');
output.print('Определим абсолютную и относительную погрешности результата:<br>');
output.print('ΔС =  '+ansv.toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+' – '+(calcansv/0x1000000*(2**shift)).toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+' = '+(ansv-(calcansv/0x1000000*(2**shift))).toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+'<br>');

output.print('<table style="border: none;">');
output.print('<tr>');
output.print('<td style="border: none;" rowspan="2">δС =&nbsp;</td>');
output.print('<td style="border: none;border-left:1px solid;" rowspan="2">&nbsp;</td>');
output.print('<td style="border: none;border-bottom:1px solid;">'+(ansv-(calcansv/0x1000000*(2**shift))).toFixed(8).replace(/\.?0+$/, '').replace(/\./, ',')+'</td>');
output.print('<td style="border: none;border-right:1px solid;" rowspan="2">&nbsp;</td>');
var pogr2 = Math.abs((ansv-(calcansv/0x1000000*(2**shift)))/(ansv))*100;
...
if (Math.abs(pogr2-pogr)<1/10**8){
  ...
  output.print('<br>');
  output.print('Погрешности результатов вызваны неточным представлением операндов. В формате Ф2 операнды представлены точнее и погрешность меньше.');
}
