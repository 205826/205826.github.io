...
output.print('<table style="border-collapse: collapse; border: 2px solid #fff; text-align:center">');
output.print('  <tr>');
output.print('    <th style="border: 2px solid #fff;">№</th>');
output.print('    <th style="border: 2px solid #fff;">Операнды<br>и действия</th>');
output.print('    <th style="border: 2px solid #fff;">СЧП (старшие)</th>');
output.print('    <th style="border: 2px solid #fff;">Множитель и<br>СЧП (младшие разряды)</th>');
output.print('    <th style="border: 2px solid #fff;">Пояснения</th>');
output.print('  </tr>');
  if(print_num){
		output.print('  <tr>');
		output.print('    <td style="border: 2px solid #fff;">1</td>');
		output.print('    <td style="border: 2px solid #fff;">2</td>');
		output.print('    <td style="border: 2px solid #fff;">3</td>');
		output.print('    <td style="border: 2px solid #fff;">4</td>');
		output.print('    <td style="border: 2px solid #fff;">5</td>');
		output.print('  </tr>');
  }
output.print('  <tr>');
output.print('    <td style="border: 2px solid #fff;">0</td>');
output.print('    <td style="border: 2px solid #fff;text-align:left;">СЧП</td>');
output.print('    <td style="border: 2px solid #fff;">00000000</td>');
output.print('    <td style="border: 2px solid #fff;">'+pb2(bBytes[0],0)+'</td>');
output.print('    <td style="border: 2px solid #fff;text-align:left;">Обнуление старших разрядов СЧП</td>');
output.print('  </tr>');
result.steps.map((x,i)=>{
  output.print('  <tr>');
  output.print('    <td style="border: 2px solid #fff;">'+(i+1)+'</td>');
  output.print('    <td style="border: 2px solid #fff;text-align:left;">'+['СЧП →','[-A]<sub>доп</sub><br>СЧП','[A]<sub>'+(ti%2!=1?'пр':'доп')+'</sub><br>СЧП<br>СЧП →'][x.operandDescription.length-1]+'</td>');
  output.print('    <td style="border: 2px solid #fff;">'+x.operandDescription.map((y,ii)=>x.operandDescription.length>1&&ii==0?'<u>'+pb(y.data.bytes[0])+'</u>':pb(y.data.bytes[0])).join('<br>')+'</td>');
  var md=x.operandDescription[x.operandDescription.length-1].data.bytes[0].data[0];
  //output.print(md);
  output.print('    <td style="border: 2px solid #fff;">'+x.operandDescription.map((y,ii)=>y.data.bytes[1]?pb2(y.data.bytes[1],y.data.cursorPosition,ii==x.operandDescription.length-1&& i<7):'').join('<br>')+'</td>');
  output.print('    <td style="border: 2px solid #fff;text-align:left;">'+([md?'Модифицированный сдвиг СЧП и множителя вправо':'Сдвиг СЧП и множителя вправо','Коррекция результата: сложение<br>старших разрядов СЧП с дополнением множимого','Сложение СЧП с множимым<br><br>'+(md?'Модифицированный сдвиг СЧП и множителя вправо':'Сдвиг СЧП и множителя вправо')][x.operandDescription.length-1])+'</td>');
  output.print('  </tr>');
});
...
result.steps.map((x,i)=>{
  output.print('  <tr>');
  output.print('    <td style="border: 2px solid #fff;">'+(i+1)+'</td>');
  output.print('    <td style="border: 2px solid #fff;text-align:left;">'+['СЧП →','!!!!!!!',(x.operandDescription[0].comment.indexOf('складываем')?'[-A]<sub>':'[A]<sub>')+(pb(x.operandDescription[0].data.bytes[0])[0]=='0'?'пр':'доп')+'</sub><br>СЧП<br>СЧП →'][x.operandDescription.length-1]+'</td>');
  output.print('    <td style="border: 2px solid #fff;">'+x.operandDescription.map((y,ii)=>x.operandDescription.length>1&&ii==0?'<u>'+pb(y.data.bytes[0])+'</u>':pb(y.data.bytes[0])).join('<br>')+'</td>');
  var md=x.operandDescription[x.operandDescription.length-1].data.bytes[0].data[0];
  //output.print(md);
  output.print('    <td style="border: 2px solid #fff;">'+x.operandDescription.map((y,ii)=>y.data.bytes[1]?pb2(y.data.bytes[1],y.data.cursorPosition,ii==x.operandDescription.length-1&& i<7):'').join('<br>')+'</td>');
  output.print('    <td style="border: 2px solid #fff;text-align:left;">'+([md?'Модифицированный сдвиг СЧП и множителя вправо':'Сдвиг СЧП и множителя вправо','!!!!!!!!!!!!!!',(x.operandDescription[0].comment.indexOf('складываем')?'Вычитание СЧП с множимым<br><br>':'Сложение СЧП с множимым<br><br>')+(md?'Модифицированный сдвиг СЧП и множителя вправо':'Сдвиг СЧП и множителя вправо')][x.operandDescription.length-1])+'</td>');
  output.print('  </tr>');
});
output.print('</table><br>');
